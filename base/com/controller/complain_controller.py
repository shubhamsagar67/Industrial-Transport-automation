...
def admin_load_complain_reply():
    try:
        if admin_login_session() == 'admin':
            ...
            complain_vo_list = complain_dao.edit_complain(
...
@app.route("/agency/view_complain")  # fetch all the data of complain and admin reply
def agency_view_complain():
    ...
    try:
        if admin_login_session() == 'agency':
            ...
            if len(complain_vo_list) != 0:
                ...
                if len(complain_vo_updated_list) == 0:
                    ...
                    return render_template("agency/addComplain.html",
        ...
        print("in agency_view_complain route exception occured>>>>>>>>>>", ex)


@app.route("/agency/insert_complain", methods=['POST'])
def agency_insert_complain():
    try:
        if admin_login_session() == 'agency':
            complain_subject = request.form.get(
                "agencyComplainSubject")  # data from complain page
            complain_description = request.form.get(
                "agencyComplainDescription")
            ...
            return redirect("/agency/view_complain")
        else:
            return admin_logout_session()

    except Exception as ex:
        print("in agency_insert_complain route exception occured>>>>>>>>>>",
              ex)
...
def user_insert_complain():
    try:
        if admin_login_session() == 'user':
            complain_subject = request.form.get(
                "userComplainSubject")  # data from complain page
...
@app.route("/agency/view_user_complain")
def agency_view_user_complain():
    try:
        if admin_login_session() == 'agency':
            complain_dao = ComplainDAO()
            complain_vo_list = complain_dao.agency_view_user_complain()
            return render_template("agency/viewUserComplain.html",
                                   complain_vo_list=complain_vo_list)
        else:
            return admin_logout_session()
    except Exception as ex:
...
@app.route("/agency/load_user_complain_reply")
def agency_load_user_complain_reply():
    try:
        if admin_login_session() == 'agency':
            complain_dao = ComplainDAO()
            complain_vo = ComplainVO()
            ...
            complain_vo_list = complain_dao.edit_complain(
                complain_vo)  # fetch the complain data
            print("complain_vo_list>>>>>", complain_vo_list)
            return render_template("agency/addUserComplainReply.html",
                                   complain_vo_list=complain_vo_list)
        else:
            return admin_logout_session()
    except Exception as ex:
        print(
            "in agency_load_user_complain_reply route exception occured>>>>>>>>>>",
            ex)


@app.route("/agency/insert_user_complain_reply", methods=['POST'])
def agency_insert_user_complain_reply():
    try:
        if admin_login_session() == 'agency':
            complain_id = request.form.get(
                "complainId")  # id from reply page for data update
            ...
            complain_vo.complain_status = "Replied"
            complain_dao.update_complain(complain_vo)
            return redirect("/agency/view_user_complain")
        else:
            return admin_logout_session()

    except Exception as ex:
        print("in agency_update_complain route exception occured>>>>>>>>>>",
              ex)


@app.route("/agency/delete_complain")  # admin delete the complain
def agency_delete_complain():
    try:
        if admin_login_session() == 'agency':
            complain_vo = ComplainVO()
            complain_dao = ComplainDAO()
