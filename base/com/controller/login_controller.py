        ...
        if len_login_list == 0:
            ...
            for row1 in login_list:
                ...
                if login_role == 'admin':
                    ...
                    print(
                        "in admin_validate_login login_secretkey>>>>>>>>>>>>>>>",
                        login_secretkey)
                    print(
                        "in admin_validate_login login_username>>>>>>>>>>>>>>>",
                        login_username)
                    return response
                elif login_role == 'agency':
                    response = make_response(
                        redirect(url_for('agency_load_dashboard')))
                    response.set_cookie('login_secretkey',
                                        value=login_secretkey,
                                        max_age=timedelta(minutes=30))
                    response.set_cookie('login_username', value=login_username,
...
@app.route('/agency/load_dashboard', methods=['GET'])
def agency_load_dashboard():
    try:
        if admin_login_session() == 'agency':
            return render_template('agency/index.html')
        else:
            return redirect(url_for('admin_logout_session'))
    except Exception as ex:
        print("agency_load_dashboard route exception occured>>>>>>>>>>", ex)
...
def admin_login_session():
    try:
        ...
        login_secretkey = request.cookies.get('login_secretkey')
        print("in admin_login_session login_secretkey>>>>>>>>>",
              login_secretkey)
        ...
        for i in global_loginvo_list:
            if login_secretkey in i.keys():
                ...
                elif i[login_secretkey]['login_role'] == 'agency':
                    login_role_flag = "agency"
...
def admin_logout_session():
    try:
        ...
        login_secretkey = request.cookies.get('login_secretkey')
        login_username = request.cookies.get('login_username')
        print("in admin_logout_session login_secretkey>>>>>>>>>",
              login_secretkey)
        print("in admin_logout_session login_username>>>>>>>>>",
              login_username)
        print("in admin_logout_session type of login_secretkey>>>>>>>>>",
              type(login_secretkey))
        print("in admin_logout_session type of login_username>>>>>>>>>",
              type(login_username))
        ...
        if login_secretkey is not None and login_username is not None:
            ...
            for i in global_loginvo_list:
                if login_secretkey in i.keys():
                    ...
                    print(
                        "in admin_logout_session global_loginvo_list>>>>>>>>>>>>>>>",
                        global_loginvo_list)
                    break
