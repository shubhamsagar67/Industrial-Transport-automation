...
class ComplainVO(db.Model):
    ...
    complain_id = db.Column("complain_id", db.Integer, primary_key=True,
                            autoincrement=True)
    complain_subject = db.Column("complain_subject", db.String(255),
                                 nullable=False)
    complain_description = db.Column("complain_description", db.String(255),
                                     nullable=False)
    complain_datetime = db.Column("complain_datetime", db.DateTime)
    complain_status = db.Column("complain_status", db.String(20))
    complain_from_login_id = db.Column("complain_from_login_id", db.Integer,
                                       db.ForeignKey(LoginVO.login_id,
                                                     ondelete='CASCADE',
                                                     onupdate='CASCADE'))
    complain_to_login_id = db.Column("complain_to_login_id", db.Integer,
                                     db.ForeignKey(LoginVO.login_id,
                                                   ondelete='CASCADE',
                                                   onupdate='CASCADE'))
    complain_reply_description = db.Column("complain_reply_description",
                                           db.String(255))
